...
export class RpgTableAddPlayerOptions {
  @StringOption({
    name: 'table-id',
  ...
  @UserOption({
    name: 'player',
    description: 'Discord user added to the table',
    required: true,
  })
...
@Injectable()
export class RpgTableAddPlayerCommand {
  constructor(
    private readonly _tableService: TableService,
    private readonly _logger: TavernaLogger,
  ) {
    this._logger.setContext(RpgTableAddPlayerCommand.name);
  }

  @Subcommand({ name: 'add-player', description: 'Add a player to an RPG table' })
  async execute(
    @Context() [interaction]: SlashCommandContext,
    @Options() options: RpgTableAddPlayerOptions,
  ): Promise<void> {
    ...
    try {
      const table = await this._tableService.addPlayer(
        options.tableId,
        options.player.username,
        options.player.id,
        interaction.user.id,
      );

      await interaction.reply(`Player <@${options.player.id}> added to table "${table.name ?? table.id}".`);
    } catch (error) {
      this._logger.error('Failed to add RPG table player from Discord command', this.normalizeError(error), {
        kind: 'audit',
      ...
      await interaction.reply({ content: 'Could not add the player to the table. Try again later.', ephemeral: true });
    }
  }
